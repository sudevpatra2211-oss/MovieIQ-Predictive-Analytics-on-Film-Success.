    ...
    if model is None or encoder is None:
        ...
        if st.button("🎯 Predict", type="primary", use_container_width=True):
            try:
                ...
                st.caption("These factors influence the prediction:")
                ...
                if budget_ratio > 0.5:
                    st.info("💰 High budget movies tend to have higher success rates")
                if pop_ratio > 0.5:
                    st.info("⭐ Popular movies tend to perform better")
                if vote_average > 7.0:
                    st.info("🌟 High-rated movies have better success odds")
